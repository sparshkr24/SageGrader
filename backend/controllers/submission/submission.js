...
const getAllMarksByFilter = async (req, res) => {
  try {
    ...
    if (markStatus == "unassigned") {
      data = await req.prisma
        .$queryRaw`SELECT s.*, mg.mentor_id FROM "Student" s
        JOIN "MentorGroup" mg ON s.id = mg.student_id
        WHERE s.id NOT IN (SELECT student_id FROM "Marks")
        `;
    } else if (markStatus == "assigned") {
      data = await req.prisma
        .$queryRaw`SELECT s.*, m.ideation, m.execution, m.pitch, mg.lock_status, mg.mentor_id
          FROM "Student" s
...
const lockSubmission = async (req, res) => {
  try {
    ...
    if (studentData.length < 3) {
      ...
      return res
        .status(403)
        .json({ error: "Please select atleast 3 students" });
    }
    ...
    for (const student of studentData) {
      if (!student.ideation || !student.execution || !student.pitch) {
        ...
        return res
          .status(403)
          .json({
            error: "Please mark all the students before locking submission",
          });
      }
      ...
      if (parseInt(mentorId) !== mentor.mentor_id) {
        return res
          .status(403)
          .json({ error: "Student not assigned under this mentor" });
      }
    }
    ...
    for (const student of studentData) {
      const emailData = {
        ...
        subject: `Assignment Graded - ${student.student_name} ${student.student_roll}`,
        text: `Your assignment has been graded by your mentor. Please login to the portal to view your marks.\nTotal Marks: ${
          student.ideation + student.execution + student.pitch
        }`,
      };
    ...
    return res.json({
      data: `Submission for mentor_id: ${mentorId} locked successfully `,
    });
  } catch (error) {
...
module.exports = { getAllMarksByFilter, lockSubmission };
